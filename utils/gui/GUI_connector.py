...
'''
class First_page:
    LOGO_PATH: str = r"torrent_icon.ico"

    def __init__(self):

        # PEER NAME.
        self.peer_name = ""
        self.half_minute_interval = 1000

        self.window = ctk.CTk()
        self.window.iconbitmap(First_page.LOGO_PATH)
        ...
        # self.window.configure(bg='#1c1d1f')
        ...
        # GRID the element.
        self.name_entry.grid(pad=0, row=1, column=0)

        # Name selection.
        def name_selection():
            chosen_name = self.name_entry.get()

            # Logic for NAME selection.
            if not chosen_name:
                self.name_entry.configure(placeholder_text='Try again...')
            else:
                self.peer_name = chosen_name
                self.window.withdraw()

        # Submit button.
        self.submit_button = ctk.CTkButton(
            master=self.window,
            text='Get into sTorrent!',
            width=200,
            anchor='center',
            font=('Arial', 20, 'bold'),
            command=lambda: name_selection(),
        )
        self.submit_button.grid(pad=0, row=2, column=0)
